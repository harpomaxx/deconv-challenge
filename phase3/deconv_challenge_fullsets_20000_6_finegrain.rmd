...
filename<-"./devcon_challenge.glmnet_results_noscale_rf_selected_features_200_20000_newmix_spearson_finegrain_caca.csv"
...
```{r message=FALSE, warning=FALSE}
filename<-"./devcon_challenge.glmnet_results_noscale_rf_selected_features_20000_20000_newmix_spearson_finegrain_caca.csv"
readr::write_csv(RESULTS,path = filename)
a11<-readr::read_csv(filename) %>%  mutate(pearson=ifelse(is.na(pearson),0,pearson),
                                      spearman=ifelse(is.na(spearman),0,spearman)
                                      ) %>% summarise(pearson_mean=mean(pearson))
a11
readr::read_csv(filename) %>%  mutate(pearson=ifelse(is.na(pearson),0,pearson),
                                      spearman=ifelse(is.na(spearman),0,spearman)
                                      ) %>% summarise(pearson_mean=mean(pearson),spearman_mean=mean(spearman))

readr::read_csv(filename) %>% filter( datasets %in% c("DS446395","DS500","FIAS4")) %>%  summarise(pearson_mean=mean(pearson),spearman_mean=mean(spearman))
...
#readr::write_csv(RESULTS,path = filename)
x8<-readr::read_csv(filename) 
x8
readr::read_csv(filename)  %>%  summarise(pearson_mean=mean(pearson),spearman_mean=mean(spearman))
readr::read_csv(filename)  %>% filter( datasets %in% c("DS446395","DS500","FIAS4")) %>%  summarise(pearson_mean=mean(pearson),spearman_mean=mean(spearman))
```
