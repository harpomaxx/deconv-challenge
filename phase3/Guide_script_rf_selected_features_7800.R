...
load("deconv_cgdata_cps_new_feat_last3.RData") # 20000 new newmix Bens'
...
# Features ranked by RF
load("results_rf_devcon_bestmodels_fgdata_cps_20000_feat_3_newmix.rdata") #mix martin
#load("results_rf_devcon_bestmodels_coarsegrain_data_cps_20000_newmix_last3.rdata") # mix with bens'
...
select_features <-function(data,label){
  ...
  best_features<-(varimp %>% arrange(desc(IncNodePurity)))[1:1500,] %>% select(feature) %>% unlist() %>% unname()
  data[which(data %>% rownames() %in% best_features),] 
}

#load("results_svr_radial_devcon_bestmodels_coarsegrain_data_cps_20000_2_noscale_selected_features_1500_newmix_spearson_last3.rdata")
load("results_svr_radial_devcon_bestmodels_fgdata_cps_20000_2_noscale_selected_features_1500_newmix_spearson.rdata")
